...
class AuthService {
    ...
    login(callback) {
        hellojs_dist_hello_all_js__WEBPACK_IMPORTED_MODULE_4__('msft').login({ scope: _lib_config__WEBPACK_IMPORTED_MODULE_3__["Configs"].scope }).then(callback, e => console.error(e.error.message));
    }
    logout() {
        this.http.post(_lib_config__WEBPACK_IMPORTED_MODULE_3__["Configs"].url + "webadmin/utils/urlslogout", {}).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_0__["map"])(response => response.json())).subscribe(response => {
        ...
        hellojs_dist_hello_all_js__WEBPACK_IMPORTED_MODULE_4__('msft').logout().then(() => { }, e => console.error(e.error.message));
    }
    hasSessiontoken() {
        const msft = hellojs_dist_hello_all_js__WEBPACK_IMPORTED_MODULE_4__('msft').getAuthResponse();
        if (msft == null) {
            return null;
        }
        else {
